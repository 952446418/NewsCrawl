...
class Movie1DownloaderMiddleware:
    # Not all methods need to be defined. If a method is not defined,
    # scrapy acts as if the downloader middleware does not modify the
    # passed objects.

    def __init__(self):
        super(Movie1DownloaderMiddleware, self).__init__()
        self.a = 0
    ...
    def process_response(self, request, response, spider):  # spider爬虫对象
        ...
        if start_url_keywords in request.url or (
        ...
        elif search_url_keywords in request.url and not search_url_ex_start_keywords in request.url:
            print('网址包含search_id=字段')
            url = request.url
            # self.changed_url_list.append(url)
